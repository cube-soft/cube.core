...
namespace Cube.Tests
{
    ...
    class SettingsFolderTest : RegistryHelper
    {
        ...
        public void Load()
        {
            using (var src = new SettingsFolder<Person>(Format.Registry, GetSubKeyName()))
            {
                src.AutoSave = false;
                src.Load();
        ...
        {
            var save = 0;
            var src  = new SettingsFolder<Person>(Format.Registry, GetSubKeyName());
            src.Loaded += (s, e) => ++save;
            src.AutoSave = false;
        ...
        public void AutoSave()
        {
            ...
            using (var src = new SettingsFolder<Person>(Format.Registry, GetSubKeyName(key)))
            {
                src.Saved           += (s, e) => ++save;
                ...
                TaskEx.Delay(TimeSpan.FromTicks(delay.Ticks * 2)).Wait();
            }
